...
public class AccessoryTests : IDisposable
{
   ...
   {
      ((Cfg)((Cfg)((Cfg)_config["accessories"]!)["mysql"]!)["options"]!)["restart"] = "on-failure";
   ...
   [Fact]
   public void ExecuteInNewContainerExcludesRestartPolicy()
   {
      ((Cfg)((Cfg)((Cfg)_config["accessories"]!)["mysql"]!)["options"]!)["restart"] = "on-failure";

      var command = Join(NewCommand("mysql").ExecuteInNewContainer(["mysql", "-u", "root"]));

      Assert.Equal(
         "docker run --rm --network kamal --env MYSQL_ROOT_HOST=\"%\" --env-file .kamal/apps/app/env/accessories/mysql.env --cpus \"4\" --memory \"2GB\" private.registry/mysql:8.0 mysql -u root",
         command);
      Assert.DoesNotContain("--restart", command);
   }
